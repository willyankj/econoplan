...
// --- UTILITÁRIO DE HASH (Removido export para corrigir o erro) ---
              ...
              for (let i = 0; i < totalInstallments; i++) {
                  ...
                  await tx.transaction.create({
                      data: {
                          ...
                          amount: installmentValue, type: type as any, date: installmentDate, workspaceId, creditCardId: cardId, categoryId: catId, 
                          isPaid: false, isInstallment: true, installmentId: installmentGroupId, installmentCurrent: i + 1, installmentTotal: totalInstallments, frequency: 'MONTHLY'
                      }
                  });
              }
          } else if (isRecurring) {
    ...
    try {
        await prisma.$transaction(async (tx) => {
            for (const t of transactions) {
                ...
                const existing = await tx.transaction.findFirst({ where: { workspaceId: account.workspaceId, OR: [ ...(externalId ? [{ externalId }] : []), ...(importHash ? [{ importHash }] : []) ] } });
                if (existing) { skipped++; continue; }
                let finalCategoryId = t.categoryId; 
                if (!finalCategoryId) {
                    const categoryName = t.categoryName || "Importados";
                    const cat = await tx.category.upsert({ where: { workspaceId_name_type: { workspaceId: account.workspaceId, name: categoryName, type: transactionType } }, update: {}, create: { name: categoryName, type: transactionType, workspaceId: account.workspaceId } });
                    finalCategoryId = cat.id;
                }
                await tx.transaction.create({ data: { description: t.description, amount: absAmount, type: transactionType, date, workspaceId: account.workspaceId, bankAccountId: accountId, categoryId: finalCategoryId, isPaid: true, externalId, importHash } });
        ...
        if (count > 0) await createAuditLog({ tenantId: user.tenantId, userId: user.id, action: 'CREATE', entity: 'Transaction', details: `Importou ${count}` });
    } catch(e) { return { error: "Erro na importação" }; }
    revalidatePath('/dashboard/transactions'); revalidatePath('/dashboard/accounts'); revalidatePath('/dashboard');
    return { success: true };
}
...
export async function getUpcomingBills() {
  ...
  const cardBills = cards.map(card => {
    const currentYear = today.getFullYear(); const currentMonth = today.getMonth(); let dueDate = new Date(currentYear, currentMonth, card.dueDay);
